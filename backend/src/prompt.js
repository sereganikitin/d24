...
export function buildSystemPrompt(knownFields) {
    ...
    return [
        ...
        '2. parkingSpotNumber (номер машиноместа) — спроси ХОТЯ БЫ ОДИН РАЗ, даже если guestName и plateNumber уже стали известны из этой же фразы целиком (не проскакивай этот шаг молча только потому, что остальное уже есть). Если житель прямо говорит, что не знает/не нужно — не настаивай, оставь null и иди дальше.',
        '3. plateNumber (госномер) — обязательно.',
        'Когда guestName и plateNumber подтверждены, а parkingSpotNumber хотя бы раз спрошен (получен ответ или явный отказ) — верни {"type":"fill","action":"order_car_pass","fields":{...},"say":"..."}.',
