        ...
        voiceAssistant = VoiceAssistant(this, binding.webView, object : VoiceAssistant.Listener {
            override fun onStateChanged(state: VoiceAssistant.State) {
                if (state == VoiceAssistant.State.IDLE) {
                    ...
                    openConcierge()
                    renderConciergeIdle()
                    // 2026-09-13: ЗДЕСЬ РАНЬШЕ был binding.webView.reload()
                    // (попытка сбросить оставшуюся открытой модалку/форму
                    // прошлого разговора, см. историю коммитов) — снят
                    // после реального бага на устройстве: перезагрузка SPA
                    // на мгновение показывает неавторизованную разметку,
                    // пока страница не подтянет сессию заново, это ловит
                    // refreshAuthState() как "экран логина" и вызывает
                    // voiceAssistant.cancel() → снова setState(IDLE) →
                    // снова reload() — бесконечный цикл "заговорил →
                    // мелькнул логин → сброс разговора → заново". НЕ
                    // возвращать reload() сюда без другого способа снять
                    // модалку (например через kiosk-inject.js, без полной
